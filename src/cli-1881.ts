...
async function search(query: string = '', type: string ='unit', page: number = 1, limit: number = 3): Promise<SearchResult<SearchContact>> {
  ...
  try {
    ...
    res.contacts.forEach(async (contact: SearchContact) => {
        ...
        lookupresult.contacts.forEach((contact: LookupContact) => {
            ...
            console.log(contact.contactPoints);
            console.log(contact.geography);

            const phone = contact.contactPoints ? `${contact.contactPoints[0].label}: ${contact.contactPoints[0].value}` : ''
            const address = contact.geography ? contact.geography.address.addressString : ''
